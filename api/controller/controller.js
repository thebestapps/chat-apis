...
exports.getAllPinPoints = function (req, res) {
  ...
  if (lat && long) {
    ...
    usercirclesModel.find(
      { $or: [{ user_id: user_id }, { circles_user_id: user_id }] },
      (err1, res1) => {
        if (err1) {
          return res.json({
            status: false,
          ...
          res1.map((item) => {
            ...
            let friend_id =
              item.user_id == user_id ? item.circles_user_id : item.user_id;
            item.users_type.map((item1) => {
              arr.push({ user: friend_id, circle_id: item1 });
            });
          });
          map_lat_longModel.find(
            {
              $and: [
                {
                  $or: [
  ...
  if (user_id && circles_user_id && users_type) {
    user.findOne({ _id: user_id }, function (err, data) {
      if (err) {
        ...
        user_circle_model.find(
          {
            $or: [
              { user_id: user_id, circles_user_id: circles_user_id },
              { user_id: circles_user_id, circles_user_id: user_id },
            ],
          },
          (err1, arr1) => {
            if (err1) {
              ...
              if (arr1.length > 0) {
                if (
                  Array.isArray(arr1[0].users_type) &&
                  arr1[0].users_type.includes(users_type)
                ) {
                  res.send({
                    code: 200,
                    message:
                      `User is already in your ` + users_type + ` circle.`,
                    data: arr1[0],
                  });
                } else {
                  let a = Array.isArray(arr1[0].users_type)
                    ? arr1[0].users_type
                    : [arr1[0].users_type];
                  user_circle_model.updateOne(
                    {
                      user_id: arr1[0].user_id,
                      circles_user_id: arr1[0].circles_user_id,
                    },
                    { users_type: [...a, users_type] },
                    (err2, arr2) => {
                      if (err2) {
                        res.send({
                          code: 400,
    ...
    user_circle_model
      ...
      .then(function (data) {
        ...
        var storageArray = data.map(async function (id) {
          ...
          var allUsersProfileData = await user
            .findOne({ _id: id.circles_user_id })
            .exec();
          circleUsers = {
            circle_users_data: allUsersProfileData,
            circle_users_type: id.users_type,
          };
...
exports.getOldChatList = function (req, res) {
  if (req.body.user_id) {
    chat
      .find(
        { $or: [{ userOne: req.body.user_id }, { userTwo: req.body.user_id }] },
        "userOne userTwo"
      )
            ...
            if (err) {
              ...
              notification_history
                .find(
                  ...
                  "status message route type actions time date active admin request createdAt"
                )
            ...
            user_circle_model.find(
              ...
              (err1, arr1) => {
                if (err1) {
                  ...
                  user.find(
                    { _id: { $in: friends } },
                    { username: 1, email: 1, country: 1, profile_pick: 1 },
                    (err2, arr2) => {
...
exports.join_group_find = async (req, res) => {
  try {
    ...
    let find_group = await chat_group
      .findOne({ pin_id: pin_id }, { chat: 0 })
      .populate("members");
    let only_chat = await chat_group.findOne(
      { pin_id: pin_id },
      { members: 0, chat: { $slice: -100 } }
    );

    console.log("chat data", find_group);
    if (find_group) {
      console.log("============ group found ==============");
      let finding_user = find_group.members
      ...
      await userUpdateChatGroup.save();
      let freshData = await chat_group
        .findById(newData._id)
        .select({ chat: { $slice: -100 }, members: 0 });
      return res.json({
        success: 1,
        chat: freshData,
      });
...
exports.group_chat_list = async (req, res) => {
  try {
    if (req.body.user_id) {
      let lists = await user
        .findById(req.body.user_id, "chat_group_id")
        .populate("chat_group_id");
      if (!lists) {
        return res.json({
          success: 0,
          message: "user not found",
